...
-- Step 1: Retrieve the crime scene report from the database
SELECT * FROM crime_scene_report;

-- Step 2: Filter the report based on the information provided: murder, jan 15,2018, SQL City
SELECT *
...
-- Step 3: Let's find the first witness from the person table
SELECT *
FROM person
...
-- Step 4: Let's find the second witness based on details provided
SELECT * 
FROM person
...
-- Step 5: Get the witnesses Statements from the Interview Table
SELECT * 
FROM interview
WHERE person_id IN (14887,16371);

/*
The first witness with ID: 14887 said "I heard a gunshot and then saw a man run out. 
...
-- Step 6: Let's deep dig using the first witness interview statement
SELECT *
...
-- Step 7: Let's check the get_fit_now_check_in based on what the second witness said
SELECT membership_id, check_in_date
FROM get_fit_now_check_in
WHERE check_in_date = 20180109

-- Step 8: To find the suspect, based on gym membership and checkin date, we use inner join
SELECT 
    gm.*,
    gc.check_in_date
...
-- Step 9: Let's save our result as a new table
...
/*
Now let's verify the actual Murderer based on license plate id. 
We can get addtional information from the persoons table
*/

-- Step 10: Verify license_id 
...
-- Step 12: Let's review the murderer's statement
...
*/
-- Step 13: Let's dive deep to find the real villain
SELECT * FROM drivers_license
WHERE gender = 'female'
...
-- Step 14: Get personal details of the potential female Villains
...
-- Step 15: Now let's check which of them attend the SQL symphony 3x in december 2017
...
-- Step 16: Now, let's find the master name of the mastermind 
SELECT p.*,
count(*) as number_of_events_attended,
fb.event_name
FROM person AS p
